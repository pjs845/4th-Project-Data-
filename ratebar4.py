...
fig.patch.set_facecolor('#f7e6da')

colors = sns.color_palette('flare',len(sorted_rate))
plt.subplot(1,2,1)
plt.barh(range(len(sorted_rate)), list(sorted_rate.values()) , color=colors, alpha=0.5, edgecolor='white')
plt.yticks(range(len(sorted_rate)), list(sorted_rate.keys()))
plt.xlabel("평균 평점")
plt.title("<별점>")

colors = sns.color_palette('flare',len(sorted_review))
plt.subplot(1,2,2)
plt.barh(range(len(sorted_review)), list(sorted_review.values()), color=colors, alpha=0.5, edgecolor='white')
plt.yticks(range(len(sorted_review)), list(sorted_review.keys()))
plt.xlabel("평균 리뷰수")
plt.title("<리뷰>")

plt.suptitle("<<지역별 캠핑장 별점 & 리뷰수>>", fontsize=17, fontweight='bold')
